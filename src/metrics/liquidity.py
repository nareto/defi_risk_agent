...
class ExoticAssetExposureOutput(BaseMetricOutput):
    ...
    metric_description: str = "The percentage of a wallet's value held in assets that are outside the top 100–200 by market capitalization or have a very short history."
...
class PortfolioConcentrationOutput(BaseMetricOutput):
    ...
    metric_description: str = "Calculated on the wallet's holdings to measure diversification. A score near 1 signifies high concentration in a single asset."
    hhi_score: float
...
def metric_calculate_exotic_asset_exposure(
    ...
    if total == 0:
        return ExoticAssetExposureOutput(percentage_exposure=0, value_explanation="Wallet empty")
    exotic_val = sum(
    ...
    return ExoticAssetExposureOutput(
        ...
        value_explanation=f"{pct:.2f}% of value in assets ranked >200 or unranked.",
    )
...
def metric_calculate_portfolio_concentration(
    ...
    if total == 0:
        return PortfolioConcentrationOutput(hhi_score=0, value_explanation="Portfolio is not diversified at all")
    hhi = 100*sum((v / total) ** 2 for v in data.asset_values)
    return PortfolioConcentrationOutput(hhi_score=hhi, value_explanation="HHI diversifiaction index for portfolio")
